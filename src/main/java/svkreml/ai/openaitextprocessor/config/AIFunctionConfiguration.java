...
//     @Tool(name = "listOwners", description = "List the owners that the pet clinic has")
    ...
    @Description("Add Pet to the database, if Owner do not exist it will be created automatically, Write owner name in latin letters")
    @Bean
    public Function<PetAndHisOwner, PetAndHisOwner> addNewPet() {
        return request -> {
            ...
            petsByOwner.computeIfAbsent(owner, k -> new LinkedHashSet<>()).add(pet);
    ...
    public Function<Owner, Pets> getPetsByOwner() {
        return request -> {
            Set<Pet> pets = petsByOwner.getOrDefault(request, Set.of());
            log.info("getPetsByOwner: {}, owner: {}", pets, request);
            return new Pets(pets.stream().toList());
        };
    }
